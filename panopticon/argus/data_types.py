...
class Detection:
    ...
    `via_light_tracker` distingue une détection CONFIRMÉE par le Detector
    (frame lourde) d'une position simplement EXTRAPOLÉE par le tracker léger
    (frame légère, mode "detect_and_track" — cf. argus/config.py::TrackingModeConfig).
    Toujours False en mode "total" (comportement inchangé). Les modules
    consommateurs (ROSTER, ORACLE, PULSE_TRACK) peuvent choisir de traiter
    différemment une position extrapolée — champ informatif, ARGUS ne
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "via_light_tracker": self.via_light_tracker,
        }
    ...
    def from_dict(payload: dict) -> "DetectionEvent":
        detections = [
            Detection(
                ...
                via_light_tracker=d.get("via_light_tracker", False),
            )
